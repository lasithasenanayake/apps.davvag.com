WEBDOCK.component().register(function(exports) {
    ...
    exports.vue= {
        data: d, methods: {
            load: load, page: page, open: open, decide: decide
        }
        , onReady: function() {
    ...
    function decide(item, action) {
        var reason=window.prompt('Learner-visible explanation (optional)', '');
        if(reason===null)return;
        var note=window.prompt('Private staff note (optional)', '');
        if(note===null)return;
        call('DecideEnrolment', {
            attempt_id: Number(item.id), action: action, reason: reason, internal_note: note
        }
        , load);
    }
}
