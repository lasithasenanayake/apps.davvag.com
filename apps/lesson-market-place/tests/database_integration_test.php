...
scoped(function() {
    ...
    $course=$db->insert('course_manager_course',['title'=>'Isolated course']);$cohort=$db->insert('course_manager_classgrade',['name'=>'September cohort','course_id'=>(int)$course,'capacity'=>100,'status'=>'active']);$subject=$db->insert('course_manager_subject',['title'=>'Isolated subject','course_id'=>(int)$course,'teacher_id'=>99]);
    ...
    $cohortEnrollment=$db->one('SELECT * FROM course_manager_enrollment WHERE class_grade_id=? AND student_id=?','ii',[(int)$cohort,1]);check($cohortEnrollment && $cohortEnrollment->access_scope==='package_lessons','activation adds learner to the Course Manager cohort without broad lesson access');
