...
require_once dirname(__DIR__,2)."/lib/CreditCheckoutService.php";
class CreditApiService {
    ...
    public function postStartCheckout($req,$res){return $this->call($res,function()use($req){$p=CreditServiceSupport::profile();$b=CreditServiceSupport::body($req);return (new CreditCheckoutService())->start($p->id,CreditServiceSupport::value($b,'order_id',0),CreditServiceSupport::value($b,'return_to',''));});}
    public function postReconcileCheckouts($req,$res){return $this->call($res,function(){CreditServiceSupport::requireAdmin();return (new CreditCheckoutService())->reconcilePending();});}
    public function postPurchaseStatus($req,$res){return$this->call($res,function()use($req){$p=CreditServiceSupport::profile();$b=CreditServiceSupport::body($req);$row=(new CreditCheckoutService())->reconcile(CreditServiceSupport::value($b,"order_id",0),$p->id);if(!$row)throw new CreditException("Purchase order was not found.");return$row;});}
