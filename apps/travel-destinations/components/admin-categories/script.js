WEBDOCK.component().register(function(exports){
    ...
    function blank(){return{id:null,name:"",description:"",sort_order:0,is_active:true,marker_key:""};}
    function load(){viewState.loading=true;api.services.GetCategories({}).then(function(r){viewState.loading=false;if(r&&r.success){replaceItems(viewState.items,r.result||[]);}else{viewState.error=msg(r,"Categories could not be loaded.");}}).error(function(){viewState.loading=false;viewState.error="Categories could not be loaded.";});}
    function save(){if(viewState.saving){return;}viewState.saving=true;viewState.error="";api.services.SaveCategory(JSON.parse(JSON.stringify(viewState.form))).then(function(r){viewState.saving=false;if(r&&r.success){reset();load();}else{viewState.error=msg(r,"Category could not be saved.");}}).error(function(){viewState.saving=false;viewState.error="Category could not be saved.";});}
    function seed(){if(viewState.seeding){return;}viewState.seeding=true;api.services.SeedReferenceData({}).then(function(r){viewState.seeding=false;if(r&&r.success){replaceItems(viewState.items,(r.result&&r.result.categories)||[]);}else{viewState.error=msg(r,"Categories could not be seeded.");}}).error(function(){viewState.seeding=false;viewState.error="Categories could not be seeded.";});}
